...
class Access:
    ...
    def check_user(self, user_role:str):
        ...
        def check_user_wrapper(func:Callable):
            ...
            def wrapper(*args, **kwargs):
                ...
                if User.role_access(info.get("token"), user_role):
                    ...
                    self.loger.warning("URAccount", f"User access denied. User with token: {info.get('token')}")
                    return "You don't have enough rights"
            return wrapper
        return check_user_wrapper
    
    def check_user_and_robot_data(self, user_role:str):
        ...
        def check_user_wrapper(func:Callable):
            ...
            def wrapper(*args, **kwargs):
                ...
                if User.role_access(info.get("token"), user_role) and Robot.robot_access(robots, info.get("Robot"), info.get("Code")):
                    ...
                    self.loger.warning("URAccount", f"User access denied to create account. User with token: {info.get('token')}")
                    return "You don't have enough rights"
            return wrapper
        return check_user_wrapper
    ...
    def check_robot_user_prog(self, user_role:str):
        from server_functions import User, Robot
        from API.multi_robots_system import URMSystem
            ...
            def wrapper(*args, **kwargs):
                ...
                if robots[info.get("Robot")]["Program"] == "":
                    if User.role_access(info.get("token"), user_role) and Robot.robot_access(robots, info.get("Robot"), info.get("Code")):
                        ...
                        self.loger.warning("URSystem", f"User access denied to set robot {info.get('Robot')} minimal angles. User with token: {request.form.get('token')}")
                        return "You don't have enough rights"
                else:
                    self.loger.warning("URSystem", f"The robot executes an automatic program. It is currently not possible to change the MinAngles parameter. User with token: {info.get('token')}")
