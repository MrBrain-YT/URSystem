...
import __robots_cache as robots_cache
from __server_functions import *
import __server_token
import __programs_starter
from __loger import Loger, Robot_loger
...
def GetFrames():
    info = request.form
    if role_access(info.get("token"), "administrator", users):
        return str(frames)
    else:
...
def GetFrames():
    info = request.form
    if role_access(info.get("token"), "user", users) or is_robot(info.get("token"), users):
...
def GetFrames():
    info = request.form
    if role_access(info.get("token"), "administrator", users) or is_robot(info.get("token"), users):
        frames[info.get("id")] = info.get("config")
        SaveToCache(Robots, tools, frames)
        update_users()
        return "True"
    else:
        return "You don't have enough rights"
...
def ImportCache():
    info = request.form
    if role_access(info.get("token"), "SuperAdmin", users):
...
def ExportCache():
    info = request.form
    if role_access(info.get("token"), "SuperAdmin", users):
        with open("__robots_cache.py", "r") as file:
            cache = file.read()
...
def GetRoleAccount():
    info = request.form
    if info.get("server_token") == __server_token.reg_token:
        update_users()
        if info.get("name") in users:
...
def URLog():
    if role_access(request.form.get("token"), "user", users):
        update_users()
...
def URLogs():
    info = request.form
    if role_access(info.get("token"), "user", users):
...
def Tools():
    if role_access(request.form.get("token"), "administrator", users):
        update_users()
        return str(tools)
    else:
...
def BindKinematics():
    info = request.form
    if role_access(info.get("token"), "administrator", users) and robot_access(Robots, info.get("Robot"), info.get("Code")):
...
def GetCurentPosition():
    info = request.form
    if is_robot(info.get("token"), users):
...
def SetRobotEmergency():
    info = request.form
    if (role_access(info.get("token"), "user", users) and \
    robot_access(Robots, info.get("Robot"), info.get("Code"))) or is_robot(info.get("token"), users):
        ...
        while True:
            if Robots[info.get("Robot")]["Emergency"] == "True":
                ...
                if Robots[info.get("Robot")]["RobotReady"] == "False":
                    ...
                    if check_angles(info, Robots) == False:
...
def HomePosition():
    info = request.form
    if role_access(info.get("token"), "user", users) and robot_access(Robots, info.get("Robot"), info.get("Code")):
        if check_angles(info, Robots) == False:
            Robot_loger(info.get("Robot")).error(f"Values ​​are not validated")
            return "Values ​​are not validated"
        else:
...
def SetProgramRun():
    info = request.form
    if role_access(info.get("token"), "System", users):
...
if __name__ == "__main__":
    ...
    # Starting UPStarter
    ups = Thread(target=lambda:__programs_starter.UPS())
    ups.start()
